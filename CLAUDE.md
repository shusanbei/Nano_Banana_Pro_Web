...
**Smart token AI (Banana Pro AI)** — 跨平台 AI 图片生成应用，支持 Gemini 和 OpenAI 标准接口。
...
21. **设置弹窗字段组边界**：`SettingsModal.tsx` 继续负责标签页状态、provider 切换、保存、测试连接和跨 section 草稿状态；provider 连接字段（Base URL、API Key、显隐按钮、Smart token推荐入口和警告/提示插槽）由 `desktop/src/components/Settings/ProviderConnectionFields.tsx` 维护。继续拆分设置弹窗时一次只抽一个清晰 section 或 field group，并保持现有保存/测试/切换行为和 Tailwind 样式不变
